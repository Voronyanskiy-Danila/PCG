...
#include "dxlib/Diligent/DescriptorHeap.hpp"
#include "dxlib/Diligent/DefaultRawMemoryAllocator.hpp"
...
DescriptorHeapAllocationManager::DescriptorHeapAllocationManager(IMemoryAllocator&                 Allocator,
    ...
    DescriptorHeapAllocationManager //
    {
        ...
        ParentAllocator,
        ThisManagerId,
        [&HeapDesc]() -> ComPtr<ID3D12DescriptorHeap> //
        {
            auto pDevice = gfw::GameFramework::Get()->GetDevice();
...
DescriptorHeapAllocationManager::DescriptorHeapAllocationManager(IMemoryAllocator&      Allocator,
    ...
    m_DescriptorSize             {gfw::GameFramework::Get()->GetDescriptorHandleIncrementSize(m_HeapDesc.Type)},
...
DescriptorHeapAllocationManager::~DescriptorHeapAllocationManager()
{
    ...
    // TODO: make right shutdown order for all systems and components
...
DescriptorHeapAllocation DescriptorHeapAllocationManager::Allocate(uint32_t Count)
{
    ...
    return DescriptorHeapAllocation{m_ParentAllocator, m_pd3d12DescriptorHeap.Get(), CPUHandle, GPUHandle, Count, static_cast<uint16_t>(m_ThisManagerId)};
}
...
CPUDescriptorHeap::CPUDescriptorHeap(IMemoryAllocator&           Allocator,
    ...
    m_HeapPool       (STD_ALLOCATOR_RAW_MEM(DescriptorHeapAllocationManager, DefaultRawMemoryAllocator::GetAllocator(), "Allocator for vector<DescriptorHeapAllocationManager>")),
    m_AvailableHeaps (STD_ALLOCATOR_RAW_MEM(size_t, DefaultRawMemoryAllocator::GetAllocator(), "Allocator for unordered_set<size_t>")),
    m_HeapDesc
    {
        Type,
    ...
    m_DescriptorSize{ gfw::GameFramework::Get()->GetDevice()->GetDescriptorHandleIncrementSize(Type)}
// clang-format on
{
    // Create one pool
    m_HeapPool.emplace_back(m_MemAllocator, *this, 0, m_HeapDesc);
    m_AvailableHeaps.insert(0);
}

CPUDescriptorHeap::~CPUDescriptorHeap()
{
    // TODO: fix delete order
...
DescriptorHeapAllocation CPUDescriptorHeap::Allocate(uint32_t Count)
{
    ...
    if (Allocation.IsNull())
    {
        ...
        m_HeapPool.emplace_back(m_MemAllocator, *this, m_HeapPool.size(), m_HeapDesc);
...
void CPUDescriptorHeap::Free(DescriptorHeapAllocation&& Allocation)
{
    ...
    StaleAllocation stale = StaleAllocation{std::move(Allocation), *this};
...
GPUDescriptorHeap::GPUDescriptorHeap(IMemoryAllocator&           Allocator,
    ...
    m_pd3d12DescriptorHeap
    {
        [&]
        {
              ...
              gfw::GameFramework::Get()->GetDevice()->CreateDescriptorHeap(&m_HeapDesc, __uuidof(pHeap), &pHeap);
              return pHeap;
        }()
    },
    m_DescriptorSize           { gfw::GameFramework::Get()->GetDevice()->GetDescriptorHandleIncrementSize(Type)},
    m_HeapAllocationManager    {Allocator, *this, 0, m_pd3d12DescriptorHeap, 0, NumDescriptorsInHeap},
    m_DynamicAllocationsManager{Allocator, *this, 1, m_pd3d12DescriptorHeap, NumDescriptorsInHeap, NumDynamicDescriptors}
// clang-format on
{
}
...
DynamicSuballocationsManager::DynamicSuballocationsManager(IMemoryAllocator&  Allocator,
    ...
    m_Suballocations  (STD_ALLOCATOR_RAW_MEM(DescriptorHeapAllocation, DefaultRawMemoryAllocator::GetAllocator(), "Allocator for vector<DescriptorHeapAllocation>")),
...
DescriptorHeapAllocation DynamicSuballocationsManager::Allocate(uint32_t Count)
{
    ...
    //VERIFY(ManagerId < std::numeric_limits<uint16_t>::max(), "ManagerID exceed allowed limit");
